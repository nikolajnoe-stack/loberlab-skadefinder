...
window.llCloseModalOnOverlay = window.llCloseModalOnOverlay || function(e) {
  ...
  // ============================================
  var I = [
    { id: 'runners_knee', n: 'Løberknæ', f: 'Patellofemoralt smertesyndrom', a: 'knee', loc: ['front', 'below', 'outer'], ch: ['dull', 'sharp'], ti: ['during', 'after', 'start'], on: ['gradual'], ag: ['stairs_down', 'hills_down', 'squatting', 'sitting', 'better_warmup'], de: 'Smerte omkring knæskallen.', w: { mild: 6, moderate: 10, severe: 16 } },
    { id: 'itb_syndrome', n: 'IT-båndssyndrom', f: 'Iliotibial bånd syndrom', a: 'knee', loc: ['outer'], ch: ['sharp', 'burning'], ti: ['during', 'after_distance'], on: ['gradual'], ag: ['hills_down', 'stairs_down', 'after_distance'], de: 'Smerte på ydersiden af knæet.', w: { mild: 6, moderate: 10, severe: 16 } },
  ...
  function renderProgress() {
    ...
    if (s.step >= 7) { pe.classList.add('ll-hidden'); return; }
    pe.classList.remove('ll-hidden');
    fe.style.width = Math.min((s.step / 6) * 100, 100) + '%';
  }
  ...
  function rLoc() {
    ...
    return '<h2 class="ll-step-title">' + (q[s.answers.area] || 'Hvor præcist?') + '</h2>' +
      ...
      '<button class="ll-btn-back" onclick="window.llGoBack(0)">Tilbage</button>';
  }
  ...
  function rSev() {
    ...
    return '<div class="ll-mb-6"><p class="ll-step-desc">' + inj.n + '</p>' +
      ...
      '<div class="ll-demo-box">' +
      '<p class="ll-demo-title">Lidt om dig</p>' +
      '<p class="ll-demo-desc">Køn og alder har stor betydning for prognosen</p>' +
      '<div class="ll-input-row">' +
      '<select class="ll-input ll-select" id="ll-demo-gender" onchange="window.llUpdateDemo()">' +
  ...
  // ============================================
  // STEP 9: PROGNOSE (ren wow-moment)
  // ============================================
  function rProg() {
    var inj = s.results[0];
  ...
  // ============================================
  // STEP 10: PLAN-TEASER MED BLURRED INDHOLD
  // ============================================
  function rPlan() {
    var inj = s.results[0];
  ...
  // ============================================
  // STEP 11: OPKALDS-UPSELL EFTER EMAIL
  // ============================================
  function rUpsell() {
    var inj = s.results[0];
  ...
  function render() {
    ...
    switch (s.step) {
      case 0: c.innerHTML = rArea(); break;
      case 1: c.innerHTML = rLoc(); break;
      case 2: c.innerHTML = rChar(); break;
      case 3: c.innerHTML = rTime(); break;
      case 4: c.innerHTML = rOnset(); break;
      case 5: c.innerHTML = rDur(); break;
      case 6: c.innerHTML = rAgg(); break;
      case 7: c.innerHTML = rDiag(); break;
      case 8: c.innerHTML = rSev(); break;
      case 9: c.innerHTML = rProg(); break;
      case 10: c.innerHTML = rPlan(); break;
      case 11: c.innerHTML = rUpsell(); break;
      case 12: c.innerHTML = rFinal(); break;
    }
  }
  ...
  // EVENT HANDLERS
  // ============================================
  window.llSelectArea = function(id) { s.answers.area = id; s.step = 1; render(); };
  ...
  window.llSelectCharacter = function(id) { s.answers.character = id; s.step = 3; render(); };
  window.llSelectTiming = function(id) { s.answers.timing = id; s.step = 4; render(); };
  window.llSelectOnset = function(id) { s.answers.onset = id; s.step = 5; render(); };
  window.llSelectDuration = function(id) { s.answers.duration = id; s.step = 6; render(); };
  ...
  window.llSubmitEmail = function() {
    ...
    track('plan_email_submitted', { injury: top ? top.id : null });

    sendToSheets({
      type: 'plan',
      name: nm, email: em,
      plan_template: planMapping[top ? top.id : ''] || '_default',
      timestamp: new Date().toISOString(), page_url: window.location.pathname
    });
            ...
            data: {
              ...
              attributes: {
                ...
                properties: {
                  ...
                  skadefinder_area: s.answers.area || '', skadefinder_source: 'plan',
                  plan_template: planMapping[top ? top.id : ''] || '_default',
                  wants_call: false
                }
              }
  ...
  window.llBookCallUpsell = function() {
    ...
    sendToSheets({
      name: s.leadInfo.name, email: s.leadInfo.email,
      injury_name: top ? top.n : '', match_score: top ? top.matchScore : 0,
      severity: s.severity || '', gender: s.gender || '',
      birthdate: s.birthdate || '', age: age || '',
      area: s.answers.area || '', location: s.answers.location || '',
      duration: dur ? dur.label : '', type: 'plan_and_call',
      timestamp: new Date().toISOString(), page_url: window.location.pathname
    });
  ...
  window.llSkipCall = function() {
    ...
    sendToSheets({
      type: 'no_call',
      name: s.leadInfo ? s.leadInfo.name : '',
      email: s.leadInfo ? s.leadInfo.email : '',
      timestamp: new Date().toISOString(),
      page_url: window.location.pathname
    });
  ...
  window.llBookCallComplex = function() {
    ...
    sendToSheets({
      type: 'complex', area: s.answers.area || '', location: s.answers.location || '',
      timestamp: new Date().toISOString(), page_url: window.location.pathname
    });
    var url = new URL(C.confirmationUrl);
