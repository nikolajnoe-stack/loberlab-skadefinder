...
window.llpCloseModalOnOverlay = window.llpCloseModalOnOverlay || function(e) {
  ...
  function sendToSheets(data) {
    ...
    console.log('[Løbe-tjekket] Sending to sheets:', data.type, data);
    fetch(C.sheetsWebhook, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    }).then(function(r) {
      console.log('[Løbe-tjekket] Sheets response:', r.status);
    }).catch(function(e) {
      console.error('[Løbe-tjekket] Sheets error:', e);
    });
  }
  ...
  function scoreColor(val) {
    if (val <= 30) return '#DC673C';
    if (val <= 60) return '#EBFA53';
    return '#27ae60';
  }
  ...
  function rDemo() {
    return '<h2 class="ll-step-title">Lidt om dig</h2>' +
      ...
      '<input type="date" class="ll-input" id="ll-perform-birthdate" placeholder="Fødselsdato" value="' + (s.birthdate || '') + '" onchange="window.llpUpdateDemo()" max="2010-01-01" min="1930-01-01">' +
      '</div>' +
  ...
  function rResult() {
    ...
    return '<div class="ll-center ll-mb-4">' +
      ...
      '<button class="ll-btn ll-btn-primary" onclick="window.llpGoToGuide()">Se din personlige guide' +
      '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 8h10M9 4l4 4-4 4" stroke-linecap="round" stroke-linejoin="round"/></svg></button>' +

      '<div class="ll-other-matches ll-mt-6">' +
      '<p class="ll-other-label">Ikke helt dig? Vælg en anden guide:</p>' +
  ...
  function rFinal() {
    return '<div class="ll-center">' +
      ...
      '<p class="ll-disclaimer">Løbe-tjekket er et vejledende værktøj baseret på dine svar. Det erstatter ikke individuel rådgivning fra en træner eller fysioterapeut.</p>' +
  ...
  window.llpSubmitEmail = function() {
    ...
    var kd = {
      data: {
        type: 'subscription',
        attributes: {
          custom_source: 'Løbe-tjekket Guide',
          profile: {
            data: {
              type: 'profile',
              attributes: {
  ...
  window.llpOpenModal = function() {
    if (!mo) {
      ...
      if (!mo) { console.error('Løbe-tjekket: Modal overlay not found'); return; }
    }
